...
@section('content')
    <div class="panel">
        ...
        <p class="muted">Perbarui identitas dasar yang digunakan di SIAMI.</p>
        ...
        @php
            ...
            $photoUrl = $user->profile_photo_path ? route('profile.photo.show', $user) : null;
        @endphp
        ...
        <div class="profile-photo-card">
            ...
            <div>
                ...
                <div class="actions">
                    <button class="button secondary" type="button" data-open-photo-focus @disabled(! $user->profile_photo_path)>Atur Sorotan</button>
                    @if ($user->profile_photo_path)
                        <form method="post" action="{{ route('profile.photo.destroy') }}" onsubmit="return confirm('Hapus foto profil?');">
                            @csrf
                            @method('delete')
                            <button class="button secondary" type="submit">Hapus Foto</button>
                        </form>
                    @endif
                </div>
                @if ($user->profile_photo_path)
        ...
        <form method="post" action="{{ route('profile.update') }}" class="form-grid" enctype="multipart/form-data">
            @csrf
            @method('patch')
    ...
    <div class="panel">
        ...
        <p class="muted">Gunakan kata sandi minimal 8 karakter.</p>
    ...
    <script>
        (() => {
            ...
            const applyFocus = () => {
                ...
                document.querySelectorAll('.topbar-actions .avatar').forEach((avatar) => {
